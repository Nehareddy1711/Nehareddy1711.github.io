...
const validateUser = async () => {
  ...
  let str = `
      ...
      <p onclick='showData(5)'><i class="bi bi-ui-checks"></i> ToDo </p>
...
const showData = async (pageId) => {
  if (pageId === 1) {
    ...
    divContent.innerHTML = await ToDo();
  }else if (pageId === 6) {
    location.reload();
  }
};
...
const getAlbums = async () => {
  ...
  json.map((element) => {
    str += `<p><b>UserId:</b>${element.userId}</p>
            <p><b>Id:</b>${element.id}</p>
            <p onclick='getPhotos(${element.id})'><b>Tittle:</b>${element.title}</p>
 `;
  });
  str += "</div";
  return str;
